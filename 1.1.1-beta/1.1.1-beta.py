...
def validate_id_card(id_number):
    ...
    # 3. 校验码计算
    # 3.1 定义系数数组（国家标准GB 11643-1999规定的系数）
...
def find_region_info(region_code):
    try:
        ...
        if not match.empty:
            ...
            city = match.iloc[0]['地市级'] if pd.notna(match.iloc[0]['地市级']) else "-"
            district = match.iloc[0]['县区级'] if pd.notna(match.iloc[0]['县区级']) else "-"
            source = match.iloc[0]['数据来源'] if pd.notna(match.iloc[0]['数据来源']) else "-"
        ...
        # 10. 从内置数据中查找或返回默认值
        return builtin_data.get(region_code, ("-", "-", "-", "-"))
...
status_bar = tk.Label(
    root,
    text="中国居民身份证验证系统 | 基于GB 11643-1999标准",  # 状态文本
    font=("微软雅黑", 10),  # 字体
    bg="#d9e6f2",  # 背景色（浅蓝色）
    fg="#333333",  # 文字颜色（深灰）
    bd=1,  # 边框宽度
